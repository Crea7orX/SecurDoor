...
#include "SecurDoor_Globals.hpp"       // for logger, APIConfig, etc.
#include "SecurDoor_TimeKeeping.hpp"   // so we can call fetchRemoteTimestamp()
#include "SecurDoor_DeviceControl.hpp" // for panic(), isLocked, etc.
#include "SecurDoor_MessagingService.hpp"
#include "SecurDoor_RSAKeys.hpp"      // for RSA keys
                                          ...
                                          const bool &displayBusy)
{
    ...
    if (headers.length() > 0) {
        ...
        while (index < headers.length()) {
            ...
            if (nextIndex == -1) nextIndex = headers.length();
                                      ...
                                      DeviceController &deviceController)
{
    ...
    if (millis() - lastCheck > 1000) {
        lastCheck = millis();
        localTimestamp++;       // originally lastTimestamp++
        lastFetch++;
        ...
        if (err) {
            ...
            if (responseDoc["isLocked"].as<bool>()) {
                ...
                if (deviceController.isLocked() != deviceController.lastLockedState()) {
                    ...
                    delay(1000);
                    if (!deviceController.isEmergency()) lcdDisplay.standbyDisplay();
...
// int fingerid, devicecontroller, timekeeping, messagingservice;
